...
// Extend Request to include file and fileType from multer
...
const uploadFile = asyncHandler(async (req: Request, res: Response) => {
  ...
  const fileType = uploadReq.fileType; // Set by multer fileFilter
 ...
 * @access Private (requires authentication)
 */
const getUploadStats = asyncHandler(async (req: Request, res: Response) => {
  const uploadsDir = path.join(__dirname, '../../uploads');
